...
//	}
	...
	//Pagination
	@GetMapping("/admin/movieList")
	public String getAllCreatedMovieList(Model model,@RequestParam("page") Optional<Integer> page, 
        ...
        Page<Movie> moviePage = movieService.findPaginated(movieList,PageRequest.of(currentPage - 1, pageSize));
	...
	public String editMovie(@ModelAttribute Movie movie,@RequestParam("file") MultipartFile file,
		...
		Movie movie1 = movieService.findMovie(movieId);
		String poster = movie1.getMoviePoster();
	...
	public String movieDetails(Model model, HttpSession session, @PathVariable int mid) {
		 ...
		 */
		String movieIdToPython = Integer.toString(m.getMovieId());
		System.out.println("Controller for python getMovieTitlesFromPython:"+movieIdToPython);
		...
		for(int i=0; i < recommendationList.size(); i++)
			...
			if(movieDetails.getMovieId()!=mid)
	...
	/*
	 * Give Selected 4 movie for recommendation
	 */
	@GetMapping("/movies/getSentimental/{user_id}")
	public String getSentimentalMovieListFromPython(@PathVariable(value = "user_id") String user_id, Model model, HttpSession session) {
		UserSession usession = (UserSession) session.getAttribute("usession");
		System.out.println("Controller for python getSentimentalMovieListFromPython :"+user_id);
		ArrayList<Integer> sentimentalReturnList = pythonIntegrationUtil.getSentimentalMovieLis(user_id);
		...
		for(int i=0; i < sentimentalList.size(); i++)
			{
			Movie movieDetails = movieService.findMovie(sentimentalList.get(i));
			sentimentalUIList.add(movieDetails);
			if(sentimentalUIList.size()==4)
				break;
		}
		...
		return "member_home"; // Full Screen Loading
	}

	/*
	 * Give Selected 16 Movie for Recommendation
	 */
	@GetMapping("/movies/getAllSentimental")
	public String getAllSentimentalMovieListFromPython(Model model, HttpSession session) {
		System.out.println("getAllSentimental");
		UserSession usession = (UserSession) session.getAttribute("usession");
		String user_id = Integer.toString(usession.getUser().getUserId());
		ArrayList<Integer> sentimentalReturnList = pythonIntegrationUtil.getSentimentalMovieLis(user_id);
		System.out.println("Python Return List Size: "+sentimentalReturnList.size());
		...
		return "viewall_sentiment"; // Full Screen Loading
	}
}
